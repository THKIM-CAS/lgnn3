...
from light_dlgn.model import LightDLGN2
...
def default_population(final_width: int, num_classes: int) -> int:
    if final_width % num_classes != 0:
        raise ValueError("profile final width must be divisible by num_classes")
    return final_width // num_classes
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--steps-per-class", type=int, default=4)
    parser.add_argument("--population", type=int, default=None)
    parser.add_argument("--feedback-features", type=int, default=None)
    parser.add_argument(
        "--step-widths",
        type=str,
        default=None,
...
def main() -> None:
    ...
    population = (
        ...
        else default_population(profile.widths[-1], profile.num_classes)
    )
    feedback_features = args.feedback_features if args.feedback_features is not None else population
    final_step_width = feedback_features + population
    default_step_widths = (*profile.widths[:-1], final_step_width)
    step_widths = parse_widths(args.step_widths, default_step_widths)

    model = LightDLGN2(
        image_shape=profile.image_shape,
        num_classes=profile.num_classes,
        steps_per_class=args.steps_per_class,
    ...
    run_dir = args.output_dir / "light_dlgn2" / args.dataset
    history: list[dict] = []
